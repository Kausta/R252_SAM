...
import numpy as np
import math
from analysis.deep_nets.utils import clamp, get_random_delta, project_lp
...
def perturb_weights(device, model, add_weight_perturb_scale, mul_weight_perturb_scale, weight_perturb_distr):
    with torch.no_grad():
        weights_delta_dict = {}
        for param in model.parameters():
            if param.requires_grad:
                if weight_perturb_distr == 'gauss':
                    delta_w_add = add_weight_perturb_scale * torch.randn(param.shape).to(device)  # N(0, std)
                    delta_w_mul = 1 + mul_weight_perturb_scale * torch.randn(param.shape).to(device)  # N(1, std)
                elif weight_perturb_distr == 'uniform':
                    delta_w_add = add_weight_perturb_scale * (torch.rand(param.shape).to(device) - 0.5)  # U(-0.5, 0.5)
                    delta_w_mul = 1 + mul_weight_perturb_scale * (torch.rand(param.shape).to(device) - 0.5)  # U(1 - 0.5*scale, 1 + 0.5*scale)
                else:
