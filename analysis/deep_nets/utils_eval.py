...
import analysis.deep_nets.data as data
import analysis.deep_nets.utils as utils
import analysis.deep_nets.utils_train as utils_train
import analysis.deep_nets.sam as sam
from analysis.deep_nets.sam import SAM


def attack_pgd(device, model, X, y, eps, alpha, scaler, attack_iters, n_restarts, rs=True, verbose=False,
    ...
    if cuda:
        max_loss, max_delta = max_loss.to(device), max_delta.to(device)
...
def rob_err(device, batches, model, eps, pgd_alpha, scaler, attack_iters, n_restarts, rs=True, linf_proj=True,
    ...
    for i, (X, X_augm2, y, _, ln) in enumerate(batches):
        ...
        pgd_delta = attack_pgd(device, model, X, y, eps, pgd_alpha, scaler, attack_iters, n_restarts, rs=rs,
